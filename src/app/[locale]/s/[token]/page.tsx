...
export default async function PublicSettlementPage({ params }: { params: Promise<{ token: string; locale: string }> }) {
  const { token, locale } = await params;
  const t = await getTranslations({ locale: locale as AppLocale, namespace: "public" });
    ...
    <div className="min-h-screen flex items-center justify-center p-5">
      <div className="w-full max-w-md surface-20 p-6 space-y-6">
        <div className="text-center">
          <div className="brand-mark mx-auto mb-3"><img src="/logo.webp" alt="PoolSplit" /></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="well p-4 text-center">
            <div className="text-[12px] text-muted mb-1">{t("expenses")}</div>
            <div className="money text-[18px] font-bold">{formatDH(settlement.totalExpenses)}</div>
          </div>
          <div className="well p-4 text-center">
            <div className="text-[12px] text-muted mb-1">{t("paid")}</div>
        ...
        {transfers.length > 0 ? (
          <div className="space-y-2">
            <h3 className="text-[15px] font-semibold">{t("transfers")}</h3>
            {transfers.map(t => (
          ...
          <div className="text-center py-4">
            ...
            <div className="text-[14px] font-semibold text-success">{t("settledUp")}</div>
          </div>
        )}

        <div className="text-center text-[12px] text-muted pt-4 border-t border-border">
          {t("createdWith")} <Link href="/" className="text-brand hover:underline font-medium">PoolSplit</Link>
        </div>
      </div>
    </div>
  );
